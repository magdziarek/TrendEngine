...
def call_polytrend_polygon(
    dataset, alpha, n, number_of_pixels, band_name, ndvi_threshold
):
...
def do_polytrend(parameters):
    ...
    annualNdvi = ee.ImageCollection.fromImages(
        list_of_years_and_collections.map(calculateAnnualMean)
    )
